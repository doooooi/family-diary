...
function renderCalendarTab(){
  ...
  let cellsHtml = cells.map(d=>{
    ...
    const ev = state.schedule[dateStr];
    const activeMembers = ev ? members.filter(m => ev.members && ev.members[m.id]) : [];
    return `<div class="day-cell ${isToday?'today':''}" data-date="${dateStr}">
...
function renderScheduleModal(dateStr){
  ...
  overlay.innerHTML = `
    <div class="sheet">
      ...
      <div class="field-block">
        <div class="field-label">몇 시에 모여요? <span class="req">필수</span></div>
        <input type="time" class="time-input" id="ev-time" value="${escapeAttr(ev.time||'')}"/>
        <div class="error-text" id="time-error">시간을 입력해야 등록할 수 있어요</div>
      </div>
      ...
      <button class="close-btn" id="close-sheet">저장하고 닫기 ✔</button>
  ...
  overlay.addEventListener('click', (e)=>{ if(e.target===overlay){ trySave(); } });
  ...
  function trySave(){
    ...
    if(!hasContent && !time){
      ...
      state.schedule[dateStr] = { time, members: selectedMembers, note };
    }
    saveSchedule();
    state.openScheduleDay = null;
    overlay.remove();
